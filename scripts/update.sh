...
IMAGE="maffia-game:${REV}-${NODE_ARCH}"
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT INT TERM
sed "s|image: maffia-game:latest|image: $IMAGE|" k8s/deployment.yaml > "$TMP/deployment.yaml"
cp k8s/service.yaml "$TMP/service.yaml"

echo "Cel ellenorizve: $TARGET ($NODE_ARCH); namespace: $NAMESPACE"

if [ "$DRY_RUN" -eq 1 ]; then
  $KUBECTL apply -n "$NAMESPACE" --dry-run=server -f "$TMP/deployment.yaml" -f "$TMP/service.yaml" >/dev/null
...
STATE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/nicqx-apps/$TARGET/maffia-game"
mkdir -p -m 700 "$STATE_DIR"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
$KUBECTL get deployment/maffia-game service/maffia-game-service -n "$NAMESPACE" -o yaml \
...
$KUBECTL apply -n "$NAMESPACE" -f "$TMP/deployment.yaml" -f "$TMP/service.yaml"
$KUBECTL rollout status deployment/maffia-game -n "$NAMESPACE" --timeout=180s
echo "Ready: Deployment/maffia-game ($IMAGE)"
